...
//     Destination.create({name: "Test", creator: "coll", image: image})
//         .then(function(){
                                                        ...
                                                        if(token){
                                                            ...
                                                            Account.findById(decode.id)
                                                                .then(function(account){
                                                                    ...
                                                                    //     refF: refF
                                                                    // })
                                                                    res.json({destinations, types, total, totalPage, services, refF})
                                                                })
                                                                .catch(function(err){
                                                                    console.log(err)
                                                                })
                                                    ...
                                                    if(token){
                                                        ...
                                                        Account.findById(decode.id)
                                                            .then(function(account){
                                                                ...
                                                                //     refF: refF
                                                                // })
                                                                res.json({destinations, types, total, totalPage, services, refF, account})
                                                            })
                                                            .catch(next)
                                                    }
                                                    else{
                                                                        ...
                                                                        if(token){
                                                                            ...
                                                                            Account.findById(decode.id)
                                                                                .then(function(account){
                                                                                    ...
                                                                                    // })
                                                                                    res.json({destinations, account, rating, rate, types, services, tours, destinationSimilar, destinationNearby})
                                                                            ...
                                                                            // })
                                                                            res.json({destinations, rating, rate, types, services, tours, destinationSimilar, destinationNearby})
                                                                        }
                                                                    })
                                                                    .catch(function(err){
                                                                        console.log(err)
                                                    ...
                                                    if(token){
                                                        ...
                                                        Account.findById(decode.id)
                                                            .then(function(account){
                                                                ...
                                                                //     services: services
                                                                // })
                                                                res.json({destinations, tours, account, types, services})
                                                            })
                                                            .catch(function(err){
                                                                console.log(err)
                                                            })
                                                        ...
                                                        //     services: services
                                                        // })
                                                        res.json({destinations, tours, types, services})
                                                    }
                                                })                             
                                                .catch(function(err){
                                                    console.log(err)
    ...
    if(token){
        ...
        Account.findById(decode.id)
            .then(function(account){
                account = account.toObject()
                if(account.role == "ctv1"){
                    Destination.findOneAndDelete({_id: id, creator: account.username})
                        .then(function(destination){                           
        ...
        if(token){
            ...
            Account.findById(decode.id)
                .then(function(account){
                    account = account.toObject()
                    if(account.role == "ctv1"){
                        Destination.deleteMany({_id: { $in: req.body.destinations }, creator: account.username})             //Duyệt qua những id có trong destinations
                            .then(function(){
                                            ...
                                            if(token){
                                                ...
                                                Account.findById(decode.id)
                                                    .then(function(account){
                                                        ...
                                                        //     services: services
                                                        // })
                                                        res.json({destinations, account, types, services})
                                                    })
                                                    .catch(function(err){
                                                        console.log(err)
                                                    })
                                                ...
                                                //     services: services
                                                // })
                                                res.json({destinations, types, services})
                                            }
                                        })
                                        .catch(function(err){
                                            console.log(err)
...
exports.simpleFindAll = function(req, res, next){
    var a = []
    Approval.find({status: 1})
        .then(function(approvals){
            for(i = 0; i < approvals.length; i++)
            ...
            Destination.find({_id: {$in: a}})
                .then(function(destination){
                    destination = destination.map(function(destination){
...
exports.collect = function(req, res, next){
    ...
    if(token){
       ...
       Account.findById(decode.id)
        .then(function(account){
            if(account.role == "tv"){
                Destination.findOneAndUpdate({_id: req.params.id, save: {$ne: account._id}}, {$push: {save: account._id}})
                    .then(function(destination){
                        Account.findOneAndUpdate({_id: account._id, favorite: {$ne: req.params.id}}, {$push: {favorite: req.params.id}})
                            .then(function(){
...
exports.uncollect = function(req, res, next){
    ...
    if(token){
       ...
       Account.findById(decode.id)
        .then(function(account){
            if(account.role == "tv"){
                Destination.findOneAndUpdate({_id: req.params.id}, {$pull: {save: account._id}})
                    .then(function(destination){
                        Account.findOneAndUpdate({_id: account._id}, {$pull: {favorite: req.params.id}})
                            .then(function(){
                                res.json({msg: "Success"})
                            })
                            .catch(function(err){
    ...
    var token = req.header("Authorization")                                             
        if(token){
            ...
            Account.findById(decode.id)
                .then(function(account){
                    account = account.toObject()
                    Destination.find({_id: {$in: account.favorite}})
                        .then(function(destinations){
                            res.json({destinations, account})
                        })
                        .catch(next)
